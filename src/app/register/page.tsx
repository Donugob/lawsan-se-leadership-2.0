...
import { ArrowLeft, ArrowRight, CheckCircle2, Building, User, CreditCard, Briefcase, GraduationCap, ShieldCheck } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      setIsSubmitting(false);
      alert('Error initializing registration. Please try again.');
    }
  };
  ...
  return (
    <main className="min-h-screen bg-forest-900 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-xl z-10 py-12">
        <Link href="/" className="inline-flex items-center gap-2 text-forest-200 hover:text-gold-400 mb-8 transition-colors">
            ...
            <div className="flex justify-center items-center gap-4 mt-8">
              {[1, 2, 3].map((item) => (
                <div key={item} className="flex items-center">
                  ...
                  {item < 3 && (
                    <div className={`w-12 h-1 rounded-full mx-2 transition-colors duration-500 ${step > item ? 'bg-gold-500' : 'bg-forest-800'}`} />
                  )}
                </div>
              ))}
            </div>
              ...
              <AnimatePresence mode="wait">
                ...
                {step === 1 && (
                  <motion.div key="step1" variants={stepVariants} initial="hidden" animate="visible" exit="exit" className="space-y-6">
                    <div className="flex items-center gap-3 mb-8 pb-4 border-b border-forest-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div>
                        ...
                        <input {...register("firstName")} className="w-full px-4 py-3.5 rounded-2xl bg-white border border-forest-200 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent transition-all shadow-sm" placeholder="John" />
                        {errors.firstName && <p className="text-red-500 text-xs mt-1.5 font-medium">{errors.firstName.message}</p>}
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-forest-900 mb-2">Last Name</label>
                        <input {...register("lastName")} className="w-full px-4 py-3.5 rounded-2xl bg-white border border-forest-200 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent transition-all shadow-sm" placeholder="Doe" />
                        {errors.lastName && <p className="text-red-500 text-xs mt-1.5 font-medium">{errors.lastName.message}</p>}
                      </div>
                    </div>

                    <div>
                      <label className="block text-sm font-semibold text-forest-900 mb-2">Email Address</label>
                      <input {...register("email")} type="email" className="w-full px-4 py-3.5 rounded-2xl bg-white border border-forest-200 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent transition-all shadow-sm" placeholder="john@example.com" />
                      {errors.email && <p className="text-red-500 text-xs mt-1.5 font-medium">{errors.email.message}</p>}
                    </div>

                    <div>
                      <label className="block text-sm font-semibold text-forest-900 mb-2">Phone Number</label>
                      <input {...register("phone")} type="tel" className="w-full px-4 py-3.5 rounded-2xl bg-white border border-forest-200 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent transition-all shadow-sm" placeholder="08012345678" />
                    ...
                    <AnimatePresence mode="wait">
                      {isStudent ? (
                        <motion.div key="student-fields" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="space-y-5">
                          ...
                          <div>
                            <label className="block text-sm font-semibold text-forest-900 mb-2">University / Institution</label>
                            <input {...register("university")} className="w-full px-4 py-3.5 rounded-2xl bg-white border border-forest-200 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent transition-all shadow-sm" placeholder="e.g. Godfrey Okoye University" />
                            {errors.university && <p className="text-red-500 text-xs mt-1.5 font-medium">{errors.university.message}</p>}
                          </div>

                          <div>
                            <label className="block text-sm font-semibold text-forest-900 mb-2">Level of Study</label>
                            <select {...register("level")} className="w-full px-4 py-3.5 rounded-2xl bg-white border border-forest-200 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent transition-all appearance-none shadow-sm cursor-pointer">
                        ...
                        <motion.div key="prof-fields" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="space-y-5">
                          <div>
                            <label className="block text-sm font-semibold text-forest-900 mb-2">Profession / Role</label>
                            <input {...register("profession")} className="w-full px-4 py-3.5 rounded-2xl bg-white border border-forest-200 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent transition-all shadow-sm" placeholder="e.g. Legal Practitioner, Entrepreneur" />
                            {errors.profession && <p className="text-red-500 text-xs mt-1.5 font-medium">{errors.profession.message}</p>}
                          </div>

                          <div>
                            <label className="block text-sm font-semibold text-forest-900 mb-2">Organization (Optional)</label>
                            <input {...register("organization")} className="w-full px-4 py-3.5 rounded-2xl bg-white border border-forest-200 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent transition-all shadow-sm" placeholder="e.g. ABC Law Firm" />
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
                    ...
                    <div>
                      ...
                      <div className="bg-white p-6 rounded-2xl text-left border border-forest-200 mb-8 shadow-sm">
                        <div className="flex justify-between items-center py-3 border-b border-forest-100">
                          ...
                          <span className="font-bold text-forest-900">{isStudent ? 'Standard Delegate (Student)' : 'Professional Delegate'}</span>
                        </div>
                        <div className="flex justify-between items-center py-3 pt-4">
                          <span className="text-forest-600 font-medium">Total Amount</span>
                          <span className="font-heading font-extrabold text-2xl text-forest-900">₦5,000</span>
                        </div>
                      </div>
                    </div>
